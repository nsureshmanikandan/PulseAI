...
def process_upload(file_path: str, dataset_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        schemas = {tab: detect_schema(df) for tab, df in dfs.items()}
...
def _persist_results(db, dataset_id: str, schemas: dict, relationships: list) -> None:
    ...
    from backend.models.db import TabProfile, TabRelationship
    import json
    ...
    for rel in relationships:
        relationship = TabRelationship(
            dataset_id=dataset_id,
            from_tab=rel.get("from_tab", ""),
            to_tab=rel.get("to_tab", ""),
